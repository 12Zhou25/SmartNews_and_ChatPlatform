...
async def _build_user_context(user_id: int, db: AsyncSession) -> str:
    """构建用户个人数据上下文（收藏+历史），注入到系统提示中"""
    parts = []

    # 获取收藏列表（最近20条）
    try:
        fav_rows, fav_total = await favorite_crud.get_favorite_list(db, user_id, page=1, page_size=20)
        if fav_total > 0:
            fav_items = []
            for news_obj, fav_time, fav_id in fav_rows:
                fav_items.append(f"- [{news_obj.id}] {news_obj.title}（{news_obj.category_id}类，收藏于{fav_time.strftime('%m-%d %H:%M')}）")
            parts.append(f"【用户收藏】（共{fav_total}条，以下是最近{fav_total}条）：\n" + "\n".join(fav_items))
        else:
            parts.append("【用户收藏】：暂无收藏")
    except Exception:
        parts.append("【用户收藏】：暂无法获取")

    # 获取浏览历史（最近20条）
    try:
        hist_rows, hist_total = await history_crud.get_history_list(db, user_id, page=1, page_size=20)
        if hist_total > 0:
            hist_items = []
            for news_obj, hist_time, hist_id in hist_rows:
                hist_items.append(f"- [{news_obj.id}] {news_obj.title}（浏览于{hist_time.strftime('%m-%d %H:%M')}）")
            parts.append(f"【浏览历史】（共{hist_total}条，以下是最近{hist_total}条）：\n" + "\n".join(hist_items))
        else:
            parts.append("【浏览历史】：暂无浏览记录")
    except Exception:
        parts.append("【浏览历史】：暂无法获取")

    return "\n\n".join(parts) if parts else "暂无用户个人数据"
...
async def ai_chat(
    ...
    # 构建用户个人数据上下文（收藏+历史），注入到系统提示中
    user_context = await _build_user_context(user.id, db)
    # 直接在 Python 层面替换占位符，避免 LangChain 模板引擎对用户数据中的花括号误解析
    system_prompt_text = system_prompt_text.replace("{user_context}", user_context)
